...
class DistModel(BaseModel):
    ...
    def initialize(self, opt, model='net-lin', net='alex', model_path=None, colorspace='Lab', printNet=False, spatial=False, spatial_shape=None, spatial_order=1, spatial_factor=None, is_train=False, lr=.0001, beta1=0.5):
        '''
        INPUTS
        ...
        self.model = model
        self.net = net
        self.use_gpu = len(opt.gpu_ids) > 0 #use_gpu
        ...
        if(self.model == 'net-lin'): # pretrained net + linear layer
            self.net = networks.PNetLin(use_gpu=self.use_gpu, pnet_type=net, use_dropout=True, spatial=spatial)
            kw = {}
            if not self.use_gpu:
                kw['map_location'] = 'cpu'
            ...
            self.net = networks.PNet(use_gpu=self.use_gpu, pnet_type=net)
            self.is_fake_net = True
        elif(self.model in ['L2','l2']):
            self.net = networks.L2(use_gpu=self.use_gpu, colorspace=colorspace) # not really a network, only for testing
            self.model_name = 'L2'
        elif(self.model in ['DSSIM','dssim','SSIM','ssim']):
            self.net = networks.DSSIM(use_gpu=self.use_gpu, colorspace=colorspace)
        ...
        if self.is_train: # training mode
            ...
            self.rankLoss = networks.BCERankingLoss(use_gpu=self.use_gpu)
    ...
    def forward_pair_by_layers(self, in1, in2, st, en):
            return self.net.forward_layers(in1, in2, st, en)

    def forward_single_layers(self, img, st=0, en=4):
        return self.net.forward_single_layers(img, st, en)

    def forward_single_layer(self, img, l=4):
        return self.net.forward_single_layer(img, l)
